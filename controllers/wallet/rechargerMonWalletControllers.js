const {Wallet}=require('../../models')
const rechargerMonWalletControllers=async(req,res)=>{
    const { userId } = req.params;
    const amount = req.body.balance;
    const wallet = await Wallet.findOne({ where: { userId } });

    if (wallet) {
      await Wallet.update({balance:wallet.balance += amount},{
        where:{
            id:wallet.id,
            userId:userId
        }
      }).then((_)=>{
        const message=`Rechargement éffectuer avec succèss`
         res.status(200).json({message,donnees:wallet});
      }).catch((err)=>{
        const message='Une erreur est survenue lors du dépôt d\'argent.'
        res.status(500).json({ message ,donnees:err});
      }) 
     
    } else {
      res.status(404).json({ error: 'Portefeuille non trouvé.' });
    }
}
module.exports=rechargerMonWalletControllers;