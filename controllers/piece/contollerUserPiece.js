...
const controllerUserPiece = async (req, res) => {
    ...
    try {
        ...
        if(req.files["verso"]&&req.files["recto"][0]){
            ...
            Data.retro = newFileName;

        }

        if(req.files["verso"]&&req.files["verso"][0]){
            ...
            Data.verso = newFileName;
        }

     
        await Piece.create(Data).then((piece) => {
                const message = 'Les données ont été ajoutées avec succès';
                res.status(200).json({ message, donnees: piece });
            }).catch((err) => {
...
module.exports = controllerUserPiece;
